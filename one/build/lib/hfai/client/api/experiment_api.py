...
# client
from hfai.conf.flags import STATUS_COLOR_MAP, EXP_PRIORITY
...
async def create_experiment(config: Union[str, StringIO, munch.Munch], **kwargs) -> Experiment:
    ...
    for k in ['version', 'name', 'spec.workspace', 'spec.entrypoint', 'resource.node_count']:
        check_exist(k)
    assert int(config.version) == 2, '版本出错，v2 接口需要 v2 配置文件'
    code_file = os.path.join(config.spec.workspace, config.spec.entrypoint) + ' ' + config.spec.get('parameters', '')
    assert len(code_file) <= 2047, f'运行命令 [workspace + entrypoint + params] 长度不应超过 2047'
    assert len(config.name) <= 511, f'name 长度不应超过 511'
    assert len(config.spec.workspace) <= 255, f'workspace 长度不应超过 255'
    assert ' ' not in config.spec.workspace, f'workspace 不允许有空格'
    assert isinstance(config.resource.node_count, int) and config.resource.node_count > 0, f'至少需要申请一个节点'
...
async def validate_nodes(nodes: List[str], file: str = '/marsv2/scripts/validation/zhw_validate.py', **kwargs):
    token = kwargs.get('token', mars_token())
    url = f'{mars_url()}/operating/node/validate?token={token}&file={file}&nodes={",".join(nodes)}'
    job_table = await _post_validate(url)
    return job_table


async def validate_experiment(name: str = None, id: int = None, chain_id: str = None, ranks: Tuple = (), file: str = '/marsv2/scripts/validation/validate.sh', **kwargs):
    nb_name = name
    assert not(nb_name is None and id is None and chain_id is None), '必须设置一个 nb_name/id/chain_id'
    ...
    if not all_rank:
        ...
        chosen_ranks = 'all'
    token = kwargs.get('token', mars_token())
    url = f'{mars_url()}/operating/task/validate?chosen_ranks={chosen_ranks}&token={token}&file={file}&'
    if id is not None:
        url += f'id={id}'
    elif nb_name is not None:
        url += f'nb_name={nb_name}'
